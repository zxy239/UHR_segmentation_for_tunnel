    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'img' in results:
            ...
            if not img.flags.c_contiguous:
                ...
                img = to_tensor(img)
                if 'context_img' in results:
                    context_img = np.ascontiguousarray(context_img.transpose(2, 0, 1))
                    context_img = to_tensor(context_img)
            else:
                img = to_tensor(img).permute(2, 0, 1).contiguous()
                if 'context_img' in results:
                    context_img = to_tensor(context_img).permute(2, 0, 1).contiguous()

            packed_results['inputs'] = img
            if 'context_img' in results:
                packed_results['inputs_context'] = context_img
