...
class LoadContextImage(LoadImageFromFile):
    def __init__(self,
                 ...
                 *,
                 ...
                 context_path: str = '/train_context/') -> None:
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        if self.context_path is not None:
            context_filename = filename.replace('/train/', self.context_path)
        try:
            ...
            if self.context_path is not None:
                if self.file_client_args is not None:
                    context_file_client = fileio.FileClient.infer_client(
                        self.file_client_args, context_filename)
                    context_img_bytes = context_file_client.get(context_filename)
                else:
                    context_img_bytes = fileio.get(
                        context_filename, backend_args=self.backend_args)
                context_img = mmcv.imfrombytes(
                    context_img_bytes, flag=self.color_type, backend=self.imdecode_backend)
        ...
        ###
        assert context_img is not None, f'failed to load image: {context_filename}'
        if self.to_float32:
            img = img.astype(np.float32)
            ###
            if self.context_path is not None:
                context_img = context_img.astype(np.float32)
