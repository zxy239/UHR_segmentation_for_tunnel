...
class RandomResizeContext(RandomResize):
    ...
    @staticmethod
    def _random_sample_ratio(scale: tuple, ratio_range: Tuple[float, float], scale_context: tuple):
        ...
        if scale_context is not None:
            scale_context = int(scale_context[0] * ratio), int(scale_context[1] * ratio)
            return scale, scale_context
        return scale
    ...
    def transform(self, results: dict) -> dict:
        ...
        ###
        if self.scale_context is not None:
            self.resize.scale = results['scale'][0]
            resize_scale_context = results['scale'][1]
        else:
            self.resize.scale = results['scale']
        results = self.resize(results)
        ###
        if 'context_img' in results:
            context_img = results['context_img']
            interpolation = self.resize.interpolation
            backend = self.resize.backend
            results['context_img'] = imresize(context_img, resize_scale_context, interpolation=interpolation, backend=backend)
...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        ###
        if 'context_img' in results:
            results['context_img'] = mmcv.imflip(
                results['context_img'], direction=results['flip_direction'])
...
class RandomCrop(BaseTransform):
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int], context_size: Tuple[int, int],
        ...
        # crop the image
        # pad_y = (context_size[0]-crop_size[0]) // 2
        # pad_x = (context_size[1]-crop_size[1]) // 2
        # pad_yx = ((pad_y, pad_y), (pad_x, pad_x), (0, 0))
        # img_pad = np.pad(img, pad_yx, mode='constant', constant_values=0)
        # img_context = img_pad[crop_y1:crop_y2+(context_size[0]-crop_size[0]), crop_x1:crop_x2+(context_size[1]-crop_size[1]), ...]
        # results['img_context'] = img_context
        if 'context_img' in results:
            big_y, big_x = results['context_img'].shape[:2]
            if big_y > context_size[0] and big_x > context_size[1]:
                small_y, small_x = results['img'].shape[:2]
                r_y = (big_y - small_y) // 2
                r_x = (big_x - small_x) // 2
                context_img = results['context_img']
                c_y = (context_size[0]-crop_size[0]) // 2
                c_x = (context_size[1]-crop_size[1]) // 2
                our_y = r_y - c_y
                our_x = r_x - c_x
                context_img = context_img[crop_y1+our_y:crop_y1+our_y+context_size[0], crop_x1+our_x:crop_x1+our_x+context_size[1], ...]
                results['context_img'] = context_img
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        results = self._crop_data(results, crop_size, context_size, self.allow_negative_crop)
        return results
