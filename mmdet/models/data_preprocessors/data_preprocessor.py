...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def __init__(self,
                 ...
                 context_batch_augments: Optional[List[dict]] = None,
                 ):
        super().__init__(
            mean=mean,
            std=std,
        ...
        if batch_augments is not None:
            ...
            self.context_batch_augments = nn.ModuleList(
                [MODELS.build(aug) for aug in context_batch_augments])
        else:
            self.batch_augments = None
            self.context_batch_augments = None
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if 'inputs_context' in data:
            data_context = data.copy()
            data_context['inputs'] = data_context['inputs_context']
            data_context = super().forward(data=data_context, training=training)
            inputs_context = data_context['inputs']
        ...
        if training and self.batch_augments is not None:
            ...
            if 'inputs_context' in data:
                for batch_aug in self.batch_augments:
                    inputs_context, _ = batch_aug(inputs_context, data_samples)
                    
        if 'inputs_context' in data:
            return {'inputs': [inputs, inputs_context], 'data_samples': data_samples}
        else:
