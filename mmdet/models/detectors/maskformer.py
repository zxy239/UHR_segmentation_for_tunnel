...
class MaskFormer(SingleStageDetector):
    ...
    def __init__(self,
        ...
        if self.w_context:    
            if context_cfg['light_version'] == 'effvit':
                if context_cfg['effvit_size'] == 'b2':
                    context_cfg['effvit_width_list'] = [24, 48, 96, 192, 384]
                    context_cfg['effvit_depth_list'] = [1, 3, 4, 4, 6]
                    context_cfg['effvit_dim'] = 32
                    context_cfg['lightweight_proj'] = [384, context_cfg['proj_ca'][0], context_cfg['proj_ca'][1]]
                    # context_cfg['effvit_proj_extra'] = [192, 768, 24]
                elif context_cfg['effvit_size'] == 'b1':
                    context_cfg['effvit_width_list'] = [16, 32, 64, 128, 256]
                    context_cfg['effvit_depth_list'] = [1, 2, 3, 3, 4]
                    context_cfg['effvit_dim'] = 16
                    context_cfg['lightweight_proj'] = [256, context_cfg['proj_ca'][0], context_cfg['proj_ca'][1]]
                    # context_cfg['effvit_proj_extra'] = [128, 768, 24]
                elif context_cfg['effvit_size'] == 'b3':
                    context_cfg['effvit_width_list'] = [32, 64, 128, 256, 512]
                    context_cfg['effvit_depth_list'] = [1, 4, 6, 6, 9]
                    context_cfg['effvit_dim'] = 32
                    context_cfg['lightweight_proj'] = [512, context_cfg['proj_ca'][0], context_cfg['proj_ca'][1]]
                    # context_cfg['effvit_proj_extra'] = [256, 768, 24]
                elif context_cfg['effvit_size'] == 'b0':
                    context_cfg['effvit_width_list'] = [8, 16, 32, 64, 128]
                    context_cfg['effvit_depth_list'] = [1, 2, 2, 2, 2]
                    context_cfg['effvit_dim'] = 16
                    context_cfg['lightweight_proj'] = [128, context_cfg['proj_ca'][0], context_cfg['proj_ca'][1]]
                    # context_cfg['effvit_proj_extra'] = [64, 768, 24]
                self.lightweight_encoder = EfficientViTBackbone(
            ...
            elif context_cfg['light_version'] == 'stdc2':
                self.lightweight_encoder = STDCNet1446(pretrain_model=context_cfg['stdc_pre'])
                context_cfg['lightweight_proj'] = [1024, context_cfg['proj_ca'][0], context_cfg['proj_ca'][1]]
            elif context_cfg['light_version'] == 'stdc1':
                self.lightweight_encoder = STDCNet813(pretrain_model=context_cfg['stdc_pre'])
                context_cfg['lightweight_proj'] = [1024, context_cfg['proj_ca'][0], context_cfg['proj_ca'][1]]
            # self.extra = context_cfg['extra']
            # self.version = context_cfg['ca']
            
            self.proj = nn.Conv2d(context_cfg['lightweight_proj'][0], context_cfg['lightweight_proj'][1], kernel_size=1)
            # if self.version == 'v2':
            self.ca = CAv2(context_cfg['lightweight_proj'][1], context_cfg['lightweight_proj'][2])
            # else:
                # self.ca = CAv1(context_cfg['effvit_proj'][1], context_cfg['effvit_proj'][2])
                
            # if self.extra:
                # self.proj_extra = nn.Conv2d(context_cfg['effvit_proj_extra'][0], context_cfg['effvit_proj_extra'][1], kernel_size=1)
                # if self.version == 'v2':
                    # self.ca_extra = CAv2(context_cfg['effvit_proj_extra'][1], context_cfg['effvit_proj_extra'][2])
                # else:
                    # self.ca_extra = CAv1(context_cfg['effvit_proj_extra'][1], context_cfg['effvit_proj_extra'][2])
    def add_context(self, x, context, HW):
        ...
        # if self.extra:
            # my_deform_inputs_extra = deform_inputs_extra(context)
        context = self.lightweight_encoder(context)
        
        feat_local = x[3]
        if isinstance(context, list):
            feat_context = context[3]
        else:
            feat_context = context
        ...
        result = self.ca(query=feat_local, reference_points=my_deform_inputs[0],
                    ...
                    level_start_index=my_deform_inputs[2], H=H, W=W)
        result = result.permute(0, 2, 1).reshape(B, -1, H, W)
        x[3] = result
        
        # if self.extra:
            # feat_local_extra = x[2]
            # feat_context_extra = context[2]
            # feat_context_extra = self.proj_extra(feat_context_extra)
            
            # B, C, H, W = feat_local_extra.shape
            # feat_local_extra = feat_local_extra.reshape(B, C, -1).permute(0, 2, 1)
            # feat_context_extra = feat_context_extra.reshape(B, C, -1).permute(0, 2, 1)
            
            # result_extra = self.ca_extra(query=feat_local_extra, reference_points=my_deform_inputs_extra[0],
                        # feat=feat_context_extra, spatial_shapes=my_deform_inputs_extra[1],
                        # level_start_index=my_deform_inputs_extra[2], H=H, W=W)
            # result_extra = result_extra.permute(0, 2, 1).reshape(B, -1, H, W)
            # x[2] = result_extra
